...
public class CategoryController {
    ...
    //查询整个商品分类
    @GetMapping
    public Resp<List<CategoryEntity>> queryCategoriesByLeverOrPid(
            @RequestParam(value = "level",defaultValue = "0")Integer level,
            @RequestParam(value = "parentCid",required = false)Long pid){
        List<CategoryEntity> categoryEntities =this.categoryService.queryCategoriesByLeverOrPid(level,pid);
